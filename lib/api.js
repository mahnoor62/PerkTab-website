...
apiClient.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      // Use simple "token" header instead of Authorization Bearer
      config.headers.token = token.trim();
    }
    return config;
  },
  (error) => {
...
export async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    const config = {
      ...
      params: method === "GET" ? { _t: ts, ...options.params } : options.params,
    };
...
export async function uploadFile(file) {
  ...
  try {
    const token = getAuthToken();
    console.log("uploadFile: Starting upload, token present:", !!token);
    ...
    console.log("uploadFile: Upload response received:", response.data);
    
    if (!response.data || !response.data.url) {
      console.error("uploadFile: Invalid response format:", response.data);
      throw new Error("Invalid response from upload server: missing url field");
    }
    
    console.log("uploadFile: Upload successful, URL:", response.data.url);
    return response.data;
  } catch (error) {
